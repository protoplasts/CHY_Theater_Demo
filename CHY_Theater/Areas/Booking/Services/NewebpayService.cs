...
namespace Commerce.Service
{
    public class NewebpayService : ICommerce
    {
        ...
        public NewebpayService(Theater_ProjectDbContext eCPAY_TestContext, IHttpContextAccessor accessor, string userId)
        {
        ...
        public string GetCallBack(ECPayViewModel inModel)
        {
            ...
            // 商店訂單編號
            TradeInfo.Add(new KeyValuePair<string, string>("MerchantOrderNo", inModel.MerchantTradeNo));
            // 訂單金額
            ...
            // 支付完成返回商店網址
            TradeInfo.Add(new KeyValuePair<string, string>("ReturnURL", $"{Config.GetSection("HostURL").Value}/booking/ECP/CallbackReturn?option=newbPay"));
            // 支付通知網址
            TradeInfo.Add(new KeyValuePair<string, string>("NotifyURL", $""));
            // 商店取號網址
        ...
        public string GetPeriodCallBack(ECPayViewModel inModel)
        {
            ...
            var TradeInfo = new Dictionary<string, object> {
                ...
                { "Version", "1.0"},
                // 訂單編號
                { "MerOrderNo", inModel.MerchantTradeNo},
                // 商品資訊
        ...
        public Result GetCallbackResult(IFormCollection form)
        {
            ...
            // Extract TradeNo from decryptTradeCollection
            string tradeNo = decryptTradeCollection["MerchantOrderNo"];

            // Use tradeNo to query the database
            var booking = _context.Bookings.FirstOrDefault(m => m.MerchantTradeNo == tradeNo);
            if (booking != null)
            {
                if (decryptTradeCollection["Status"] == "SUCCESS") booking.BookingStatus = "已付款";
            }

            var ecpayOrder = _context.PaymentTransactions.FirstOrDefault(m => m.MerchantTradeNo == tradeNo);
            if (ecpayOrder != null)
            {
                // Assuming Amt is the equivalent of TradeAmt in your decrypted data
                var point = (int)(Math.Round(decimal.Parse(decryptTradeCollection["Amt"]) / 100m));

                ecpayOrder.RtnCode = int.Parse(decryptTradeCollection["Status"] == "SUCCESS" ? "1" : "0"); // Assuming 1 for success, 0 for failure
                if (decryptTradeCollection["Status"] == "SUCCESS") ecpayOrder.RtnMsg = "已付款";
                ecpayOrder.PaymentDate = DateTime.ParseExact(decryptTradeCollection["PayTime"], "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture);
                // Note: SimulatePaid is not present in the decrypted data, you might need to handle this differently
                ecpayOrder.Points = point;
                _context.SaveChanges();
            }
            // Assuming CustomField2 is not available in the decrypted data, you might need to handle this differently
            var userid = ecpayOrder.MemberID;
            var rewardPoint = new RewardPoint
            {
                UserId = userid,
                Points = (int)(Math.Round(decimal.Parse(decryptTradeCollection["Amt"]) / 100m)),
                EarnedDate = DateTime.ParseExact(decryptTradeCollection["PayTime"], "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture),
                ExpiryDate = DateTime.ParseExact(decryptTradeCollection["PayTime"], "yyyy-MM-dd HH:mm:ss", CultureInfo.InvariantCulture).AddYears(1),
                IsUsed = false
            };
            ...
            result.RtnMsg = "交易成功";
            result.MerchantTradeNo = tradeNo;
            return result;
        }
